...
resource "aws_vpc" "main" {
  cidr_block           = var.vpc_cidr         # VPCのIP範囲（例: 10.0.0.0/16）
...
resource "aws_subnet" "public_1a" {
  ...
  cidr_block              = var.public_subnet_cidr    # サブネットのIP範囲（例: 10.0.1.0/24）
...
resource "aws_subnet" "private_1a" {
  ...
  cidr_block        = var.private_subnet_cidr         # 例: 10.0.2.0/24
